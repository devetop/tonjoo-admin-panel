...
export default function PublicUpload() {
    ...
    const [postProvateFile, { error, data }] = usePostPublicFileMutation()
    ...
    return (
        <Card>
            <CardHeader className={'border-b px-6 py-[14px] bg-head'}>
                <CardTitle className='text-p2'>Public File</CardTitle>
            </CardHeader>
...

                    {
                        data?.data?.file_url_path && (
                            <div className="max-w-[200] text-wrap">
                                <Image className="border p-1 rounded mx-auto" src={data?.data?.file_url_path} width={210} height={70} alt="public image preview" />
